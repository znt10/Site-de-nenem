...
def home(request):
    ...
    destaques = Produto.objects.filter(ativo=True, destaque=True).prefetch_related('imagens')[:8]
    novidades = Produto.objects.filter(ativo=True).prefetch_related('imagens')[:8]
...
def produto_lista(request):
    produtos = Produto.objects.filter(ativo=True).select_related('categoria').prefetch_related('imagens')
    categorias = Categoria.objects.all()
